...
const RepaymentPage = () => {
    ...
    /* ---------- DELETE ---------- */
    useEffect(() => {
        const handleDeleteRepayment = async (event: any) => {
            const repayment = event.detail;

            const result = await Swal.fire({
                html: `Delete repayment record for <b><i>${repayment.unitid}</i></b>?`,
                icon: 'warning',
            ...
            try {
                const res = await fetch(`/api/repayment/${repayment.unitid}`, {
                    method: 'DELETE',
                });
        ...
        window.addEventListener('deleteRepayment', handleDeleteRepayment);

        return () => {
            window.removeEventListener('deleteRepayment', handleDeleteRepayment);
        };
    }, []);
    return (
        <>
