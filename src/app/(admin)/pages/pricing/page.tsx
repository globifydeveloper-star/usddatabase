...
export const metadata: Metadata = { title: 'Pricing', other: { subTitle: 'Pages' } }

const PricingCard = ({ description, features, price, type, isPopular }: PricingType) => {
  return (
    <Card className={`h-100 ${!isPopular && 'overflow-hidden'} `}>
      <CardHeader className={`p-3 ${isPopular ? 'text-bg-success' : 'text-bg-primary'}  text-center`}>
        <h4 className="fw-bold fs-24">{type}</h4>
      ...
      <CardBody className="p-3">
        ...
        <ul className="d-flex flex-column gap-2 list-unstyled fs-15">
          {features.map((data, idx) => (
            <li key={idx} className='icons-center'>
      ...
      <CardFooter>
        <a href="" className={`btn ${isPopular ? 'btn-danger' : 'btn-outline-primary'}  btn-lg fw-semibold w-100`}>
          Buy Now
        </a>
      </CardFooter>
      {isPopular && (
