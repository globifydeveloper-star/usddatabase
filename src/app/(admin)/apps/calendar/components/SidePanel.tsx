    ...
    <>
      <div className="d-grid">
        <Button variant="primary" type="button" onClick={createNewEvent}>
          <IconifyIcon icon="bx:plus" width={14} height={14} className="me-2" />
          &nbsp;Create New Event
        </Button>
      </div>
      <div id="external-events" className='mt-2'>
        ...
        {externalEvents.map(({ id, variant, title }) => (
          <div key={id} className={`external-event bg-soft-${variant} text-${variant}`} title={title} data-class={`bg-${variant}`}>
